...
for i in range(len(matching_keys)):
    if len(x_train_file_names_positive) < 5:
...
for fold_num in range(5):
    ...
    tmp_validation_names_positive = x_train_file_names_positive[fold_num * 1:(fold_num + 1) * 1]
    ...
    for j in tqdm(range(200)):
        ...
        y = np.concatenate((np.zeros(4) + 1, np.zeros(4)), axis=0)
        X = np.array(X)
        clf = RandomForestClassifier(max_depth=2, random_state=0)
        clf.fit(X, y)
        this_fold_test_result += clf.predict_proba(test)[:, 1]
        this_fold_validation_result += clf.predict_proba(validation_X)[:, 1]
    np.savetxt("./result/random_forest_shuffle_5/fold_" + str(fold_num+1) + "_test", this_fold_test_result)
    np.savetxt("./result/random_forest_shuffle_5/fold_" + str(fold_num+1) + "_validation", this_fold_validation_result)
